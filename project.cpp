...
KNOB< BOOL > KnobOpt(KNOB_MODE_WRITEONCE, "pintool", "opt", "0", "Run optimize mode");
KNOB< UINT32 > KnobUnrollNum(KNOB_MODE_WRITEONCE, "pintool", "max-loop-unroll-num", "4", "maximal loop unrolling factor");
...
struct Loop
{
    ...
    uint64_t _count_seen; // backward edge execution count
...
// Ex 3
std::unordered_map<ADDRINT, Loop> candidates_map;
std::vector<std::pair<ADDRINT, Loop>> candidates;
...
VOID Instruction(INS ins, VOID* v)
{
    ...
    loops_profiler[ins_addr]._source_addr = ins_addr;
...
bool is_cond_jmp(INS ins)
{
    xed_decoded_inst_t *xedd = INS_XedDec(ins);
    xed_category_enum_t category_enum = xed_decoded_inst_get_category(xedd);

    return category_enum == XED_CATEGORY_COND_BR;
}
...
void get_inverse_inst(INS ins, xed_decoded_inst_t* output_inst)
{
    ...
    {
        printf("un-conditional branch found :)\n\n");
        return;
    }
...
//        }
        printf("Optimizing Loop.. JMP addr:0x%lx TARGET addr:0x%lx\n", candid.second._source_addr, candid.second._target_addr);
        optimize_loop_unrolling(rtn,
                                unroll_factor,
                                static_cast<uint64_t>(candid.second._target_addr),
                                static_cast<uint64_t>(candid.second._source_addr));
...
{
    uint32_t profiler_elements_per_row = 9;
    uint32_t profiler_source_addr_index = 0;
    uint32_t profiler_target_addr_index = 1;
    uint32_t profiler_count_seen_index = 2;
//    uint32_t profiler_rtn_addr_index = 7;
//    uint32_t profiler_inst_count_index = 8;
    ...
    while(std::getline(in_file, line))
    {
        ...
        uint64_t candid_count_seen = 0;
        IMG routine_img;
        while(std::getline(iss, segment, ','))
        {
            if (counter % profiler_elements_per_row == profiler_source_addr_index)
            {
                std::istringstream converter(segment);
                converter >> std::hex >> source_addr;
                routine_img = IMG_FindByAddress(source_addr);
                loop._source_addr = source_addr;
            }
            if (counter % profiler_elements_per_row == profiler_target_addr_index)
            {
                std::istringstream converter(segment);
                converter >> std::hex >> target_addr;
                loop._target_addr = target_addr;
            }
            // Taking the 2nd element out of 8
            if (counter % profiler_elements_per_row == profiler_count_seen_index)
            {
                std::istringstream converter(segment);
                converter >> candid_count_seen;
                // TODO: continue from here, store this within the mapping to candidates_map
                if (!IMG_Valid(routine_img) || !IMG_IsMainExecutable(routine_img))
                {
                    counter++;
                    continue;
                }
                loop._count_seen = candid_count_seen;
                candidates_map[source_addr] = loop;
            }
//            // Taking the 7'th element out of 8
//            if (counter % profiler_elements_per_row == profiler_rtn_addr_index)
//            {
//                std::istringstream converter(segment);
//                converter >> std::hex >> candid_addr;
//                routine_img = IMG_FindByAddress(candid_addr);
//            }
//            if (counter % profiler_elements_per_row == profiler_inst_count_index)
//            {
//                std::istringstream converter(segment);
...
//            }
    ...
    candidates = std::vector<std::pair<ADDRINT, Loop>>(candidates_map.begin(), candidates_map.end());
...
VOID ImageLoad(IMG img, VOID *v)
{
    ...
    if (!KnobDoNotCommitTranslatedCode)
    {
        ...
        printf("Candidate functions committed\n");
    }
    else
    {
        printf("Candidate functions were not committed\n");
    }
...
VOID Fini(INT32 code, VOID* v)
{
    ...
    {
        std::stringstream ss;
        ss << std::hex << x.second._target_addr;
        ...
        ss3 << std::hex << x.second._source_addr;
        std::string source_addr = "0x" + ss3.str();

        uint32_t routine_inst_count = routine_map[x.second._routine_addr];

        out_file << source_addr << "," << target_addr << "," << x.second._count_seen << "," << x.second._count_loop_invoked << "," << mean_taken << "," << x.second._diff_count << "," << x.second._routine_name << "," << routine_addr << "," << routine_inst_count << "\n";
    }
    out_file.close();
}
